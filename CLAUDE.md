...
- **Step isolation** — the agent sees completed step labels (marked) and the current step label (highlighted), plus a count of remaining steps. Future step labels are hidden. It sees the current step's directions and output shape, and values from prior steps interpolated into the directions. It never sees other steps' directions, future step labels, or future output shapes. This prevents agents from skipping ahead — they will try.
...
- **version** — `thinker version`, `thinker -v`, or `thinker --version` prints the version from package.json.
